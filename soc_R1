...
void loop() {
  ...
  in_voltage = adc_voltage / (R2/(R1+R2));
  ocv = in_voltage;
  if (currentMillis - startMillis >= period && soc != 0.001 && temperature < 40 && soc > 40 && ocv > 0.1 && ocv <3.9)  //test whether the period has elapsed
    {
      ...
      while (soc < 80){
        if (currentMillis - startMillis >= period){
          totalCoulumbs = totalCoulumbs + (0.01);
          soc = soc+((totalCoulumbs/3600.0)/3);
          float temperature = dht.readTemperature();
          lcd.setCursor(0,1);
          lcd.print("SOC :");
